...
void
clipcopy(const Arg *dummy __attribute__((unused)))
{
	Atom clipboard;
...
void
clippaste(const Arg *dummy __attribute__((unused)))
{
	Atom clipboard;
...
void
selpaste(const Arg *dummy __attribute__((unused)))
{
	XConvertSelection(xw.dpy, XA_PRIMARY, xsel.xtarget, XA_PRIMARY, xw.win, CurrentTime);
}

void
numlock(const Arg *dummy __attribute__((unused)))
{
